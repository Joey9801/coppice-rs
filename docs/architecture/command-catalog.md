...
| Payload | `job: Job` (id — **client-minted**, the submission's idempotency identity per ADR 0026 —, image, `requests: Resources`, `priority: i32`, `max_runtime_us: optional uint64`, `quota_entity: QuotaEntityId`, `retry: RetryPolicy { max_retries: u32, retry_user_errors: bool }`, `env: JobEnv` — an immutable `BTreeMap<String, String>` set only at submission, written on the wire as `repeated EnvVar { name, value }` in ascending name order, never a proto `map`), `multiplier: PriorityMultiplier` (Q32.32 — the API resolves the user's `priority` through the replicated multiplier table at proposal time; apply never sees the raw `i32` in arithmetic, per ADR 0019), `actor: Actor`, `submitted_at_us` |
| Validation | `abort_requested` unset; `env` satisfies `coppice_core::env::validate` (at most 64 variables, POSIX portable names, UTF-8 values, and a 32 KiB total-bytes bound). If `job.id` is already in state (including terminal jobs not yet evicted): identical client-supplied spec, `env` included, → **accepted no-op** (idempotent resubmission, no events — the retryer observes success and the original job; the no-op still re-checks the actor's submit authority at its own log position, per the revocation guarantee, but never changes `submitted_by`); different spec → `SubmitSpecMismatch`. Otherwise `quota_entity` exists and the actor holds `submitter` (or higher) over it (ADR 0023). |
...
| Rejections | `SubmitSpecMismatch`, `UnknownQuotaEntity`, `InvalidJobEnv`, `InvalidCommand` (pre-set abort flag), `PermissionDenied` |
...
| | |
| --- | --- |
| Proposer | Leader housekeeping loop — **only after the job-history-store write for every listed job is durable** (ADR 0012). That ordering is a proposal-side obligation; the apply itself just deletes. Timestamps ride in the command; apply never consults a clock to decide "72 h have passed" — the proposer decided that, measuring from each job's `terminal_at_us` (never from submission; a terminal job without the stamp is exempt). One command names at most 512 jobs, longest-overdue first, and one housekeeping pass proposes at most 16 of them sequentially — the bound on the raft entry, the apply, and the derived event batch ([coordinator-runtime.md](coordinator-runtime.md), "Housekeeping"); the rest of a backlog waits for the next tick. |
