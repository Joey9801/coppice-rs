...
- **Every external dependency is a deployment cost.** A minimal production
  cluster should need a DNS name, an OIDC issuer, and an ordinary
  externally-verifiable TLS certificate on its public listener (which any
  web-UI deployment needs regardless) — and nothing else: no external
  machine-plane/control-plane PKI, no Vault, no secrets manager on the
  critical path. External issuance remains a supported *substitution*,
  never a requirement.
...
- **Manifest absent** → this is a new instance. If it holds no usable leaf
  ...
  - an initialized cluster with a matching `cluster_id` is found →
    self-join (§6);
  - no initialized cluster is found → **park**: serve `ProbeCluster`
    and the local admin socket (§3), report the `waiting` phase through
    `/readyz` (§9), and keep
...
5. signs the supplied operator CSR into the first operator-profile
   ...
   and prints it with the CA bundle — with no CSR
...
- **Leaf profiles.** Three, per ADR 0022's framework: *coordinator*
  ...
  lint.
- **Issuance: enrollment is public ingress.** A new machine does not
  yet trust the cluster CA, so placing enrollment behind cluster-PKI
  TLS is a circular trust problem, and distributing the CA out of band
  first makes bringup two-phase. Enrollment therefore lives where the
  bootstrap trust posture is already solved: **`POST /api/v1/enroll` on
  the public client listener**, bypassing OIDC, authenticated solely by
  a role-scoped enrollment token (§5) accompanying a CSR. The response
  is the issued leaf plus the cluster CA bundle; from that moment all
  machine traffic speaks cluster-CA mTLS. Enrollment is not a user
  resource API, but it is naturally an ingress endpoint: unauthenticated
  machines must reach it, and the token is its route-specific
  authentication. With `ProbeCluster` on the machine plane, this is the
  entire network surface available without a client certificate;
  formation is not a network verb at all (§3). The flow is exactly
  ADR 0011's, now with a decided signer, and applies to **coordinators
  and agents alike**: at cluster birth, coordinators 2..N enroll against
  the freshly formed cluster before joining — the same two steps (get a
...
- **Enrollment transport is the client listener's posture — explicitly
  ...
  - With TLS on the listener (the production posture: an
    externally-signed certificate, or a TLS-terminating load balancer),
    enrollment clients perform **ordinary system-root hostname
    verification** — no pin, no cluster-CA distribution, no second
    certificate mechanism, and cluster bringup is single-phase: bake
    the token into the launch template, form, and the fleet enrolls.
  - Plain HTTP requires the conspicuous **client-side**
    `[enrollment].insecure = true` opt-in and is documented as
    development/test-only, because it exposes enrollment tokens on the
    wire. This is a distinct setting from the serving side's
    `[client_tls].insecure` (below): each party declares its own
    posture, and an agent — which has no client listener of its own —
    still needs the `[enrollment]` flag to enroll against an insecure
    cluster. An enrollment token configured against an unverified
    endpoint without that flag **fails at startup**; there is no
    fallback. Trust-on-first-use, and a pinned-cluster-CA anchor
    (which reintroduced a form-then-publish two-phase bringup), were
    both considered and rejected: the listener either has verifiable
    TLS or the deployment has explicitly accepted insecurity.
  - A TLS-terminating load balancer is part of the trusted credential
    path: the backend hop carries bearer tokens and must be secured
    accordingly — the same standing the LB already has for user bearer
    tokens.
- **Ingress hardening for `/enroll`.** The tradeoff is stated, not
  hidden: this places a security-sensitive machine endpoint on public
  ingress, weightiest for the coordinator token, which is
  root-equivalent (§5). It is accepted because the endpoint is
  authenticated by a purpose-built high-entropy bearer token, gains the
  hardening below, and the alternative was a second
  certificate-distribution mechanism plus another listener to secure.
  The token is accepted only in a
  header or redacted body field and must never be logged — by the
  application, proxy, WAF, or tracing stack. Request-size, concurrency,
  and rate limits apply *before* CSR processing; authentication
  failures are uniform (no token-validity oracle); cookies and CORS are
  disabled for the route. A follower receiving an enrollment request
  proxies it internally to the leader over the mTLS admin channel — it
  never redirects a client carrying a token.
- **Renewal stays on the machine plane.** Re-issuance is authenticated
  by the *current* leaf over the already-mTLS internal services —
  agents renew via the agent service, coordinators via the admin/raft
  service — so the public listener never needs machine
  client-certificate authentication and `/enroll` is for certless
  first contact only. Renewal preserves the subject, making short-lived
  leaves free, and is a *policy decision*: the leader refuses renewal
  for identities an operator has marked revoked. With short leaf
  lifetimes this is v1's certificate revocation mechanism — there is
  no CRL or OCSP distribution, and an already-issued leaf remains
  valid until it expires. The `[tls]` paths remain hot-reloaded (mtime
  watch, plus SIGHUP to force), served via a connection-time
  certificate resolver on all listeners and picked up by outbound
  channels on reconnect; in-flight connections finish on the old leaf.
...
- **Client (HTTP API) listener: externally-signed TLS or explicit
  ...
  front), or plain HTTP under a conspicuous `[client_tls].insecure =
  true` opt-in — the server-side counterpart of, and a separate setting
  from, the enrollment client's `[enrollment].insecure` above. A cluster-issued leaf as a client-listener default
  ...
  the mTLS machine plane (raft, agent gateway) is where the cluster CA
  serves. This same posture is what `/enroll` (above) rides on: hosting
  enrollment here reuses the one listener whose bootstrap trust is
  already solved, instead of adding another certificate-distribution
  mechanism, listener, port, load-balancer rule, and firewall policy.
  Should a deployment later require network-level separation for
  enrollment, an **optional dedicated enrollment bind address** —
  same protocol, same externally-verifiable server certificate — is
  the deferred escape hatch; it changes no clients and never becomes
  part of the default deployment. One role the cluster CA does keep on
  this listener, independent of the serving certificate: it remains
  the *client-certificate* trust anchor against which ADR 0022's
  operator-profile certificates are verified. User authentication
  otherwise remains OIDC bearer (ADR 0022); the listener never
  *requires* machine client certificates.
...
enrolls for an agent leaf (CN = node id, ADR 0011) via the same public
`/enroll` endpoint under the same transport rules, and renews over its
existing mTLS agent session —
...
- **`GET /readyz`** on the client listener (beside the existing
  ...
  cardinality reached — desired shape, saying nothing about health),
  and any admission refusal the daemon last received (a
  duplicated-identity refusal per §7 surfaces here). Two gates, two
  questions:
...
- **A minimal production deployment needs a DNS name, an OIDC issuer,
  and one ordinary TLS certificate for its public listener** (which any
  web-UI deployment needs regardless; secure enrollment verifies against
  it). No external machine-plane/control-plane PKI, no Vault, no
  secrets manager, no per-replica
  certificate provisioning: one artifact, one byte-identical config file,
  one command line for every daemon, two enrollment tokens in launch
  templates, and one local `cluster init` at cluster birth. Bringup is
  **single-phase**: tokens are chosen before formation (seeded via
  `--policy`), enrollment rides the public listener's
  externally-verified TLS with ordinary system-root verification, and
  nothing minted at formation needs distributing to the fleet. The
  user-facing HTTP listener is externally-signed TLS or explicit
  insecure — the cluster CA serves only the mTLS machine plane, and
  `/enroll` is the one machine verb on public ingress, with its
  hardening rules and stated tradeoff in §4. Integration tests get
  N-process local clusters from the `file` backend (insecure
  enrollment) plus one harness-driven local init call. External PKI,
  a dedicated enrollment bind address, and per-instance
  short-lived-token delivery remain documented substitutions for
...
- **The cluster is its own CA; the key never enters replicated state,
  ...
  one token model, one renewal protocol over the respective mTLS
  services (agent session for agents, admin/raft service for
  coordinators); ADR 0022's operator certificate is
...
- New failure modes to test explicitly: interrupted join at every step;
  ...
  against a server failing system-root hostname verification (refusal,
  token not sent); an enrollment request landing on a follower (proxied
  internally to the leader, never a token-carrying redirect); token
  absent from application, proxy, and trace logs asserted directly;
  rate/size limits enforced before CSR processing; renewal over the
  machine-plane mTLS services (agent session, coordinator admin) and
  refused on `/enroll`; `ReplaceVoter` with a lagging `new_node_id`
...
- Deferred, unchanged in scope: signed-request and platform-CAS network
  ...
  platform, opt-in) beside the token authenticator; an optional
  dedicated enrollment bind address (same protocol and certificate) for
  deployments wanting network-level separation of `/enroll`; the cluster-held
