...
mtime poll. Under `[tls] source = "cluster"` the path is the fixed
cluster-managed layout (renewal would install the bundle too, but only
over a session the agent can still open, which is exactly what a
stranded agent cannot do):

```
scp coord-1:/var/lib/coppice/pki/ca.crt agent-7:/var/lib/coppice-agent/pki/ca.crt
```

Under `[tls] source = "external"` the daemon never writes its material
at all (no renewal, no trust-anchor adoption — see
[security.md](security.md) and [configuration.md](configuration.md)), so
the operator is the entire distribution path, and the destination is
whichever `ca_path` that agent's config names.
...
(An agent's material lives at the same layout under
`/var/lib/coppice-agent/pki` instead. Either way, under
`[tls] source = "external"` substitute whatever paths that node's config
names.)
...
(This recovery applies to `[tls] source = "cluster"`, where enrollment
re-runs against the fixed data-dir layout. Under `source = "external"`
there is no re-enrollment to fall back to — replace the operator-provisioned
files at that node's configured paths instead.)
...
5. **Re-enroll the fleet.** Under `[tls] source = "cluster"`, every agent
   needs the material under its PKI directory removed so startup
   enrollment re-runs; under `source = "external"` there is nothing to
   remove — provision fresh operator-issued material for the new cluster
   instead. Either way, every agent and coordinator needs the new
   `cluster_id` in its config. Collect a fresh operator credential from
   the `init` output.
