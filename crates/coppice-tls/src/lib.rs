...
/// name or SAN identity) and its numeric port (ADR 0037 §4).
///
...
pub struct TlsStore {
    ...
    /// raft peer-channel map, ADR 0037 §4) record the generation they dialed at
...
impl TlsStore {
    ...
    /// re-dials when this advances (ADR 0037 §4), so a rotation is never pinned
    /// to a stale channel.
    ///
...
// ---------------------------------------------------------------------------
// Endpoint verification (ADR 0037 §4)
// ---------------------------------------------------------------------------

/// Dial `addr` (`host:port`) over mTLS using `store`'s current material and
/// return the *serving* leaf's subject (ADR 0037 §4 endpoint verification).
///
