...
#[tokio::test]
async fn an_established_connection_survives_a_rotation() {
    // The in-flight-connections half of the ADR 0037 §4 contract: a stream
    // whose handshake completed on the old leaf keeps carrying bytes after the
    // material rotates; only NEW connections pick up the new leaf.
    let dir = tempfile::tempdir().unwrap();
    let ca = Ca::new();
    let (cert1, key1) = ca.leaf("server");
    let paths = write_material(dir.path(), &cert1, &key1, &ca.pem);
    ...
    // Establish a connection on the old leaf and keep BOTH halves open.
    let (client_cert, client_key) = ca.leaf("client");
    let cfg = client_config(&ca.pem, Some((&client_cert, &client_key)));
    let connector = TlsConnector::from(Arc::new(cfg));
    let tcp = TcpStream::connect(addr).await.unwrap();
    let name = ServerName::try_from("localhost").unwrap();
    let mut client_stream = connector.connect(name, tcp).await.unwrap();
    let mut server_stream = tokio::time::timeout(Duration::from_secs(5), incoming.next())
        .await
        .expect("accept within timeout")
        .expect("an incoming connection")
        .expect("handshake ok");
    {
        ...
        let seen = conn
            ...
            .map(|c| c.as_ref().to_vec())
            .expect("server presented a certificate");
        assert_eq!(seen, parse_certs(&cert1)[0].as_ref());
    }

    // Rotate the leaf on disk and reload the store under the live listener.
    std::thread::sleep(Duration::from_millis(10));
    let (cert2, key2) = ca.leaf("server");
    std::fs::write(&paths.cert, &cert2).unwrap();
    std::fs::write(&paths.key, &key2).unwrap();
    ...
    // The established stream still carries bytes both ways on the OLD leaf.
    ...
    tokio::io::AsyncReadExt::read_exact(&mut server_stream, &mut buf)
        .await
        .expect("established connection still readable after rotation");
    assert_eq!(&buf, b"ping");
    server_stream.write_all(b"pong").await.unwrap();
    server_stream.flush().await.unwrap();
    tokio::io::AsyncReadExt::read_exact(&mut client_stream, &mut buf)
        .await
        .expect("established connection still writable after rotation");
    assert_eq!(&buf, b"pong");

    // Meanwhile a NEW connection is served the rotated leaf.
    let cfg = client_config(&ca.pem, Some((&client_cert, &client_key)));
    let seen_new = client_handshake_server_cert(addr, cfg).await.unwrap();
    let _ = incoming.next().await;
    assert_eq!(seen_new, parse_certs(&cert2)[0].as_ref());
}
