...
/// so the health a read derives is the verdict the monitor reaches (give or
/// take a housekeeping tick).
const HEARTBEAT_LIVENESS_DEADLINE: std::time::Duration = std::time::Duration::from_secs(90);
...
fn node_health(mark: Option<&LivenessMark>) -> dto::NodeHealth {
    match mark {
        None => dto::NodeHealth::Unknown,
        Some(mark) if mark.silent_for >= HEARTBEAT_LIVENESS_DEADLINE => dto::NodeHealth::Lost,
        Some(LivenessMark {
            last_heartbeat: Some(_),
            ..
        }) => dto::NodeHealth::Healthy,
        Some(_) => dto::NodeHealth::Unknown,
    }
}
...
fn node_summary(
    ...
    dto::NodeSummary {
        ...
        last_heartbeat: mark.and_then(|mark| mark.last_heartbeat),
...
fn cost_report(state: &StateMachine, record: &JobRecord) -> dto::CostReport {
    ...
    // has then trued up, and the net of their settlements is the job's. They
    // stay unknown when any attempt's settlement was not retained (a job
    // finished before the field existed), rather than passing the gross
    // charge off as settled.
    let (actual_ucu, true_up) = match (record.state.is_terminal(), net_true_up(state, record)) {
        (true, NetTrueUp::Settled(net)) => {
...
/// The net true-up across a job's attempts, or that it cannot be known.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum NetTrueUp {
    /// Some attempt has no retained settlement: it finished before
    /// `AttemptRecord::settlement` existed and was restored from an older
    /// snapshot, or has not finished yet. The net cannot be reported without
    /// misstating it, so nothing is.
    Unknown,
    /// Every attempt has settled: refunds less surcharges, as a single refund
    /// or surcharge, or `None` when nothing was adjusted (no attempts, or the
    /// adjustments cancel exactly).
    Settled(Option<TrueUp>),
}

/// Net the job's attempts' settlements. Each settlement is the replicated
/// record of what that attempt's charge trued up to
/// (`AttemptRecord::settlement`), so this is read, never re-derived.
fn net_true_up(state: &StateMachine, record: &JobRecord) -> NetTrueUp {
    let (mut refunds, mut surcharges) = (0u64, 0u64);
    for id in &record.attempts {
        let Some(settlement) = state.attempts.get(id).and_then(|ar| ar.settlement) else {
            return NetTrueUp::Unknown;
        };
        match settlement.true_up {
            TrueUp::Refund(c) => refunds = refunds.saturating_add(c.0),
            TrueUp::Surcharge(c) => surcharges = surcharges.saturating_add(c.0),
        }
    }
    NetTrueUp::Settled(if refunds > surcharges {
...
mod tests {
    ...
    #[test]
    fn a_node_with_no_heartbeat_marks_reads_unknown_and_never() {
        // A follower, or a leader no agent has reported to: no marks means
        // no verdict and no stamp — never a fabricated `healthy`.
        let node = NodeId::new();
        let mut state = StateMachine::default();
        state.nodes.insert(node, test_node(node));
    ...
    #[test]
    fn node_health_and_last_heartbeat_derive_from_the_leaders_marks() {
        let heard_at = ts(1_000_000_000);
        let (fresh, silent, seeded, seeded_overdue, unheard) = (
            NodeId::new(),
            NodeId::new(),
            NodeId::new(),
            NodeId::new(),
        ...
        let mut state = StateMachine::default();
        for id in [fresh, silent, seeded, seeded_overdue, unheard] {
            state.nodes.insert(id, test_node(id));
        }

        // Silent for exactly the deadline is lost — the monitor's own
        // comparison (`>=`), so a read never calls a node healthy that the
        ...
        let usage = liveness_of(&[
            ...
            // call healthy, but the grant does start the monitor's clock.
            (seeded, None, just_inside),
        ...
        assert_eq!(by_id[&fresh].health, dto::NodeHealth::Healthy);
        assert_eq!(by_id[&fresh].last_heartbeat, Some(heard_at));
        // The stamp is still served — silence is the whole story for health.
        assert_eq!(by_id[&silent].health, dto::NodeHealth::Lost);
        assert_eq!(by_id[&silent].last_heartbeat, Some(heard_at));
        assert_eq!(by_id[&seeded].health, dto::NodeHealth::Unknown);
        assert_eq!(by_id[&seeded].last_heartbeat, None);
        assert_eq!(by_id[&seeded_overdue].health, dto::NodeHealth::Lost);
        assert_eq!(by_id[&seeded_overdue].last_heartbeat, None);
        assert_eq!(by_id[&unheard].health, dto::NodeHealth::Unknown);
        assert_eq!(by_id[&unheard].last_heartbeat, None);
    }

    /// Health reads the monotonic silence and nothing else: a wall-clock
    /// step in either direction moves the displayed stamp's apparent age,
    /// never the verdict — which stays the monitor's.
    #[test]
    fn health_ignores_the_wall_stamp_entirely() {
        let (stepped_forward, stepped_back) = (NodeId::new(), NodeId::new());
        let mut state = StateMachine::default();
        state
            .nodes
        ...
        let usage = liveness_of(&[
            ...
            (
                stepped_forward,
                Some(ts(1_000_000_000) - Duration::from_secs(3600)),
                std::time::Duration::from_secs(1),
            ),
            // Stamped in the wall clock's "future" (the clock jumped back),
            // but silent past the deadline: lost.
            (
                stepped_back,
                Some(ts(1_000_000_000) + Duration::from_secs(3600)),
        ...
        let by_id: BTreeMap<_, _> = list_nodes(&state, &usage)
            ...
            .map(|n| (n.id, n))
            .collect();
        assert_eq!(by_id[&stepped_forward].health, dto::NodeHealth::Healthy);
        assert_eq!(by_id[&stepped_back].health, dto::NodeHealth::Lost);
    }

    /// A lost node leaves the cluster's schedulable capacity — the read-side
    /// half of the `DeclareNodeLost` the monitor reaches from the same marks
    /// — while its allocations keep counting until the loss is reconciled.
    #[test]
    fn overview_counts_a_silent_node_lost_and_excludes_its_capacity() {
        let now = ts(1_000_000_000);
        let (live, lost) = (NodeId::new(), NodeId::new());
        let job = JobId::new();
        let attempt = AttemptId::new();
        let alloc = AllocationId::new();

        let mut state = StateMachine::default();
        state.nodes.insert(live, test_node(live));
        state.nodes.insert(lost, test_node(lost));
        state.allocations.insert(
            alloc,
        ...
        // Only the live node's capacity; the lost node's allocation still
        // holds its funding.
        assert_eq!(capacity.capacity.cpu_millis, 4000);
    ...
    /// `settlement` is an additive snapshot field: a job that finished before
    /// it existed restores with none. Its settled figures are then unknown,
    /// never the gross charge dressed up as settled.
    #[test]
    fn get_job_cost_report_leaves_an_unsettled_terminal_attempt_unknown() {
        let now = ts(1_000_000);
        let mut state = StateMachine::default();
        let entity = quota_id(1);
        put_entity(&mut state, entity, None, "e", 1_000_000, 0, now);
        ...
        // The first attempt settled; the second predates the field.
        let mut a1 = test_attempt(
            first,
            id,
            node,
            AttemptState::Terminal(AttemptOutcome::Exited { code: 1 }),
        );
        a1.settlement = Some(Settlement {
            actual_cost: CostUnits(300),
            true_up: TrueUp::Refund(CostUnits(700)),
        });
        let a2 = test_attempt(
            second,
        ...
        assert_eq!(cost.charged_ucu, 2000);
        assert_eq!(cost.actual_ucu, None);
        assert_eq!(cost.true_up, None);
    }

    /// Adjustments that cancel exactly across retries settle to the gross
    /// charge with no net true-up — distinct from the unknown case above.
    #[test]
    fn get_job_cost_report_settles_cancelling_true_ups_to_the_gross_charge() {
        let now = ts(1_000_000);
        let mut state = StateMachine::default();
        let entity = quota_id(1);
        put_entity(&mut state, entity, None, "e", 1_000_000, 0, now);
        ...
        a1.settlement = Some(Settlement {
            actual_cost: CostUnits(1300),
            true_up: TrueUp::Surcharge(CostUnits(300)),
        });
        let mut a2 = test_attempt(
            second,
            id,
        ...
        a2.settlement = Some(Settlement {
            actual_cost: CostUnits(700),
            true_up: TrueUp::Refund(CostUnits(300)),
        });
        state.attempts.insert(first, a1);
        state.attempts.insert(second, a2);

        let cost = get_job(&state, &id, now, &no_memos()).unwrap().cost;
        assert_eq!(cost.actual_ucu, Some(2000));
        assert_eq!(cost.true_up, None);
    }
