...
pub struct HostFacts {
    ...
    /// `PRETTY_NAME`; known non-Linux families receive a label in responses.
    pub os_version: String,
    /// Human-readable kernel release, including a family label in responses.
...
impl From<&coppice_core::node::HostFacts> for HostFacts {
    fn from(f: &coppice_core::node::HostFacts) -> Self {
        // Normalize at the API boundary so older registrations are
        // self-describing without rewriting the replicated record.
        let (os_version, kernel_version) = f.normalized_versions();
        HostFacts {
            os: f.os.clone(),
            os_version,
            kernel_version,
