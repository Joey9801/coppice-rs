...
//! and semantics. Protobuf remains the canonical format for internal RPC,
...
//! - enums as PascalCase strings matching the `types.ts` unions.
...
mod tests {
    ...
    #[test]
    fn node_summary_serializes_to_the_types_ts_shape() {
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                "capacity": { "cpuMillis": 4000, "memoryBytes": 8_000_000_000u64, "diskBytes": 0 },
                "allocated": { "cpuMillis": 1000, "memoryBytes": 1_000_000, "diskBytes": 0 },
                "used": { "cpuMillis": 0, "memoryBytes": 0, "diskBytes": 0 },
                "labels": { "zone": "a" },
                "schedulable": true,
                "health": "Unknown",
                "epoch": 3,
    ...
    fn terminal_outcome_carries_kind_class_and_exit_code() {
        ...
        assert_eq!(
            json,
            serde_json::json!({ "kind": "Exited", "exitCode": 3, "class": "UserError" })
        );

        // `exitCode` is an optional property (`exitCode?`), omitted — not
        // null — when the kind has none.
        let json =
            serde_json::to_value(AttemptOutcome::from(&attempt::AttemptOutcome::NodeLost)).unwrap();
        assert_eq!(
            json,
            serde_json::json!({ "kind": "NodeLost", "class": "Platform" })
        );
    }
}
