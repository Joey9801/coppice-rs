...
pub async fn ensure_enrolled(
    ...
    // `config::load` already refuses `[enrollment]` under external provenance,
    // but a `Config` can be assembled in process too (`coppice dev`), and this
    // is the one boundary at which enrollment would *write* material. Hold
    // the invariant here as well, so externally-provisioned files are never
    // overwritten by whatever the cluster hands back (issue #127).
    if config.tls_source() == config::TlsSource::External {
        anyhow::bail!(
            "refusing to enroll: [tls] source = \"external\" names operator-provisioned \
             material, and enrollment would overwrite it (issue #127)"
        );
    }
    let paths = config.tls_paths();
