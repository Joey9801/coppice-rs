...
pub struct TlsConfig {
    // Private, like the paths: the invariant between `source` and the path
    // keys is what `validate` establishes, and an in-process caller that could
    // flip one without the others would defeat it. `Config::tls_source` reads it.
    source: TlsSource,
    #[serde(default)]
