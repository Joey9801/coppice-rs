...
use coppice_scheduler::score::{age_horizon_us, effective_score, penalty_product};
...
proptest! {
    ...
    fn score_strictly_increases_with_multiplier(
        ...
        let now = 2_000_000_000_000_000i64;
    ...
    fn score_strictly_increases_with_age(
        ...
        let now = 5_000_000_000_000_000i64;
        let younger = effective_score(PriorityMultiplier(m), penalty, now - base_horizons * H, now, H, w_age);
        let older = effective_score(
            PriorityMultiplier(m),
            penalty,
            now - (base_horizons + gap_horizons) * H,
            now,
            H,
            w_age,
        );
    ...
    fn larger_penalty_strictly_lowers_the_score(
        ...
        let now = 7_000_000_000_000_000i64;
        let submitted = now - age_frac;
    ...
    fn urgent_outranks_backlog_within_the_head_start(
        ...
        let m_hi = m_lo + dm;
        let now = 9_000_000_000_000_000i64;
        // The exact head-start in microseconds, then a gap strictly inside it.
        let head_start_us = (real(m_hi) - real(m_lo)) * H as f64 / (penalty * w_age);
        let gap = (head_start_us * frac) as i64;
    ...
    fn score_is_bit_deterministic(
        ...
        submitted in 0i64..2_000_000_000_000_000,
    ...
    fn penalty_product_is_monotone_and_deterministic(
        ...
        let entity = |usage: u64| QuotaEntity {
            ...
            usage: UsageState { usage: CostUnits(usage), last_update_us: 0 },
        };
        entities.insert(leaf, entity(usage_lo));
        let lo = penalty_product(&entities, leaf, &policy, 0);
        let lo_again = penalty_product(&entities, leaf, &policy, 0);
        prop_assert_eq!(lo.to_bits(), lo_again.to_bits());
        prop_assert!(lo >= 1.0);
        entities.insert(leaf, entity(usage_lo.saturating_add(extra)));
        let hi = penalty_product(&entities, leaf, &policy, 0);
...
proptest! {
    ...
    fn horizon_is_always_positive(
        ...
        let decay = DecayPolicy { tick_us, decay_per_tick };
        prop_assert!(age_horizon_us(&decay) >= 1);
    }
}
...
fn penalty_product_is_depth_capped() {
    ...
    let product = penalty_product(&entities, leaf, &policy, 0);
