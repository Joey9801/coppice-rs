...
mod tests {
    ...
    /// A fixture instant `micros` past the epoch. `Duration` here is
    /// `std::time::Duration` (tokio timers below), so the domain span type
    /// stays out of this module entirely.
    ...
    /// and return the full tapped `(index, at, events)` stream.
    async fn tapped_stream(
        batchings: Vec<Vec<(u64, coppice_state::Command)>>,
    ) -> Vec<(u64, Timestamp, Vec<coppice_state::Event>)> {
        let (publisher, views) =
            ViewPublisher::new(StateMachine::default(), 0, ViewPublisherConfig::default());
        ...
        while let Some(item) = tap_rx.recv().await {
            match item {
                TapItem::Batch(batch) => stream.push((batch.applied_index, batch.at, batch.events)),
    ...
    async fn event_stream_is_invariant_under_apply_batching() {
        ...
        // ways. KOI-3 / ADR 0032 (T1): the tapped `(index, at, events)`
        ...
        let expected: Vec<(u64, Timestamp, Vec<coppice_state::Event>)> = vec![
            (
                3,
                ts(100),
