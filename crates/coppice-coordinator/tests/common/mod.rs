...
impl Daemon {
    ...
    pub fn wipe_installation(&self) {
        // One call is enough now that cluster-managed material lives at
        // `<data_dir>/pki` (issue #127): wiping the volume takes the leaf with
        // it, which is exactly what a replaced instance sees.
        self.wipe_data_dir();
    }
