//! The `ec2-asg` discovery backend (ADR 0037 §2): the one platform-specific
...
//! fleet that gates its hooks on readiness (§9) would otherwise be invisible to
...
//! the network.
//!
//! Like every backend, this one has a candidate-listing role only: discovery
//! output is advisory dialing information, never membership evidence
//! (ADR 0037 §7 explicitly rejects discovery-backend liveness as evidence).
...
impl Discovery for Ec2AsgDiscovery {
    async fn candidates(&self) -> Vec<String> {
        ...
        compose_candidates(&members, &ips, self.port)
    }
}
