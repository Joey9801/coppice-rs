...
mod discovery {
    ...
    //!
    //! The backend selector, the four backend tables, and the exactly-one-table
    //! rule are `coppice-discovery`'s: the agent parses the same spelling out of
    //! its own file. What stays here are the three knobs only a coordinator has
    //! (`cluster_size`, `removal_grace`, `learner_expiry`), which is why this is
    //! a struct declared over the shared field types rather than a `serde`
    //! flatten — flattening would silently disable `deny_unknown_fields`, and a
    //! typo'd knob must still fail-stop naming the offending key.

    use std::time::Duration;

    use coppice_discovery::{
        BackendKind, DnsBackend, Ec2AsgBackend, FileBackend, SeedConfig, StaticBackend,
    };
    ...
    impl DiscoveryConfig {
        /// The portable half of this section — the selector plus its one
        /// matching table — as `coppice-discovery` wants it. Cheap clones of
        /// the small backend tables; call it at the `build` site.
        pub(crate) fn seed_config(&self) -> SeedConfig {
            SeedConfig {
                backend: self.backend,
                static_backend: self.static_backend.clone(),
                dns: self.dns.clone(),
                file: self.file.clone(),
                ec2_asg: self.ec2_asg.clone(),
            }
        }
        ...
        /// file, ADR 0037 §2). The rule itself is the shared one; only the
        /// coordinator-only knobs above are checked here.
        pub(crate) fn validate(&self) -> anyhow::Result<()> {
            self.seed_config().validate()
        }
    }
...
mod tests {
    use super::*;
    use coppice_discovery::BackendKind;
    use std::io::Write;
...
"#;
    ...
    fn full_documented_example_parses() {
        ...
        assert_eq!(
            config.discovery.seed_config().static_addrs(),
            [
    ...
    fn minimal_config_applies_documented_defaults() {
        ...
        assert_eq!(config.discovery.backend, BackendKind::Static);
        assert!(config.discovery.seed_config().static_addrs().is_empty());
        assert_eq!(config.discovery.cluster_size, 3);
