//! Bounded-channel capacities and cadence constants for the coordinator
//! runtime.
//!
...
/// Apply task -> event fanout (`docs/architecture/coordinator-runtime.md`,
/// "event tap" row). `try_send`; on full the batch is DROPPED and the
/// receiver synthesizes a gap (ADR 0008). Owned by `main::bootstrap`, which
/// constructs the tap: the apply task itself lives in `coppice-consensus`,
/// not this crate.
pub const EVENT_TAP_CAPACITY: usize = 4096;

/// Fanout -> one queue per subscriber ("per-subscriber queue" row).
/// `try_send`; on full the subscriber is marked gapped and its backlog is
/// dropped.
pub const SUBSCRIBER_QUEUE_CAPACITY: usize = 1024;

/// Session tasks -> ingestion, one shared channel ("agent inbound" row).
/// `send().await`; a full channel stalls the session's socket read, which is
/// the correct backpressure point (TCP, never apply).
...
/// `send().await`; producers are leader-only loops that tolerate this
/// backpressure.
pub const COMMAND_ROUTER_CAPACITY: usize = 1024;

/// Client -> event fanout subscribe requests (not itself a channel-inventory
/// row; the fanout task's own inbox). `send().await`, sized like the other
/// small control channels.
pub const SUBSCRIBE_REQUESTS_CAPACITY: usize = 64;

/// Fanout reconnection ring: max events retained ("fanout ring" row, ADR
/// 0008). Evict-oldest when full — it is a reconnection buffer, not history.
