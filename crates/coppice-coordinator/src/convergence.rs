...
/// Whether all three cluster-managed files exist under `<data_dir>/pki` — the
/// cheap precondition for dialing the mTLS probe plane, distinct from
/// `has_usable_leaf`'s expiry check because a probe with an expired leaf still
/// fails informatively.
