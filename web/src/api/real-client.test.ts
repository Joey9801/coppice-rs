    ...
    fetchMock.mockResolvedValueOnce(
      jsonResponse({
        ...
        bindings: [
          ...
          { role: 'submitter', scope: null, scope_path: null },
          {
            role: 'submitter',
            scope: 'quota-00000000-0000-0000-0000-000000000002',
            scope_path: 'acme/eng',
          },
        ],
  ...
  it('falls back to the email claim for a display name', async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({
        ...
        bindings: [{ role: 'operator', scope: null, scope_path: null }],
        implicit_admin: false,
      }),
    )
    const session = await createRealClient().getSession()
