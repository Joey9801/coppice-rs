...
Observability/debugging UI for the Coppice cluster. It is a Vite + React 19 + TypeScript SPA; see README.md for the human-facing overview.

**Status: all methods route through the real client; two remain
client-side unsupported stubs.** In normal mode, `src/api/index.ts`
overrides every `CoppiceApi` method with `src/api/real-client.ts`'s
implementation — none of the interface is served from the mock. Within
`real-client.ts`, `getNodeLogs` and `getCoordinatorLogs` make no HTTP
request at all; they just resolve to `{ unsupported: true }` because their
server routes still answer `501 UNIMPLEMENTED`. Setting `VITE_COPPICE_MOCK`
switches the whole client to the mock wholesale (`src/api/mock/`) instead
of overriding individual methods — that's how `npm run dev` works without
a coordinator, and how tests run. Check `real-client.ts` and
`src/api/index.ts`'s delegation table directly before assuming a method's
current backing — see "How to implement an unsupported endpoint" below for
the two stubbed methods, or "How to add a new endpoint" for anything not
yet on `CoppiceApi` at all.
...
## How to implement an unsupported endpoint

`getNodeLogs` and `getCoordinatorLogs` are already implemented in
`real-client.ts` and already delegated to in `src/api/index.ts` — they
just resolve to `{ unsupported: true }` client-side because their server
routes still answer `501 UNIMPLEMENTED`. Once a route lands server-side,
there's no delegation-table change needed; replace that method's stub
body in `real-client.ts` with a real `fetch` implementation, following the
wire-mapping conventions the other methods in that file already use
(snake_case keys and enum strings ↔ the camelCase/PascalCase `types.ts`
shapes, `Date` ↔ ISO 8601 strings, error translation via `ApiError`).

## How to add a new endpoint

This is the path for a method that doesn't exist on `CoppiceApi` yet, or
one still served by the mock. The server-side contract (route table,
consistency params, error codes, JSON conventions) is fixed by ADR 0031
...
3. Add that method to the delegation table in `src/api/index.ts`
   (`{ ...mock, listJobs: real.listJobs }`) — needed only here, for a
   method not already delegated to `real`.
